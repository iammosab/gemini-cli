...
describe('ExtensionRegistryClient', () => {
  ...
  it('should return extensions sorted alphabetically', async () => {
    ...
    // extension-one, extension-three, extension-two
    const result = await client.getExtensions(1, 3, 'alphabetical');
    expect(result.extensions).toHaveLength(3);
    expect(result.extensions[0].id).toBe('ext1'); // extension-one
    expect(result.extensions[1].id).toBe('ext3'); // extension-three
    expect(result.extensions[2].id).toBe('ext2'); // extension-two
  });

  it('should return extensions in random order', async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => mockExtensions,
    });

    // Mock Math.random to return a predictable sequence for testing shuffle
    // This is a simple check to ensure it's not just returning the default order if we can force a swap
    const randomSpy = vi.spyOn(Math, 'random').mockReturnValue(0.5);

    const result = await client.getExtensions(1, 3, 'random');
    expect(result.extensions).toHaveLength(3);
    // With 0.5, the shuffle will likely change the order.
    // We just want to ensure it runs without error and returns all items.
    // A strict order check with random is flaky without precise mocking of every call.
    // But we can check that it contains all elements.
    expect(result.extensions.map((e) => e.id).sort()).toEqual([
      'ext1',
      'ext2',
      'ext3',
    ]);
